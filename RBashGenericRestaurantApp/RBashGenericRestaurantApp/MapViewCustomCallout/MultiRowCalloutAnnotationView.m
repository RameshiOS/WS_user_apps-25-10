...
//        _titleLabel.layer.borderWidth=1;
        ...
        if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
            _titleLabel.font= [UIFont fontWithName:@"Thonburi-bold" size:12.0];
            [_titleLabel setMinimumScaleFactor:14.0/[UIFont labelFontSize]];

        }
        else
        {
            _titleLabel.font= [UIFont fontWithName:@"Thonburi" size:12.0];
            [_titleLabel setMinimumScaleFactor:8.0/[UIFont labelFontSize]];
        }
    ...
    if (annotation)
    {
        _cellOffsetY = 25 + (2*kMultiRowCalloutCellGap);
...
- (void)prepareContentFrame
{
    ...
    if (_titleLabel)
    {
        ...
        if (UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPad) {
             _titleLabel.frame = CGRectMake(_cellInsetX, 10, kMultiRowCalloutCellSize.width+30 - (2*_cellInsetX), 25);
        }
        else
        {
              _titleLabel.frame = CGRectMake(_cellInsetX, 10, kMultiRowCalloutCellSize.width+30 - (2*_cellInsetX), 25);
    ...
    {
//        cell.frame = CGRectMake(0, _cellOffsetY + index * (kMultiRowCalloutCellSize.height+kMultiRowCalloutCellGap), kMultiRowCalloutCellSize.width - (_cellInsetX*2), kMultiRowCalloutCellSize.height - (2*kMultiRowCalloutCellGap));
        
           cell.frame = CGRectMake(_cellInsetX-10, _cellOffsetY + index * (kMultiRowCalloutCellSize.height+kMultiRowCalloutCellGap),_titleLabel.frame.size.width+10 , kMultiRowCalloutCellSize.height - (2*kMultiRowCalloutCellGap));
